...
import { Shield, Loader2, AlertCircle, RefreshCw, Key, Package, Check, X, Pencil, Save } from "lucide-react";
import { Button } from "../ui/button";
import { getErrorMessage } from "@/lib/utils";
import { useQuery } from "@tanstack/react-query";
import { fetchRolePermissions, RbacRole, RolePermissionModule, UpdateRolePermissionsInput } from "@/lib/rbac";
...
function RolePermissionsDialog({
  ...
  const [localPermissions, setLocalPermissions] = useState<PermissionState>(new Map());
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {isLoading ? (
            ...
            <div className="space-y-4" role="list" aria-label="Module permissions">
          ...
          {isEditMode ? (
            <>
              ...
              <Button
                ...
                aria-label={isSaving ? "Saving permissions" : "Save permission changes"}
              >
                {isSaving ? (
                  <>
        ...
        >
          {permissionTypes.map((type) => {
            ...
            const hasPermission = isEditMode
              ? localPermissions.get(permission.permissionId) ?? permission.hasPermission
            ...
            return (
              <button
                key={type}
                type="button"
                onClick={() => isInteractive && onPermissionToggle(permission.permissionId)}
