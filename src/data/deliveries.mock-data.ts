...
let deliveryDetails: DeliveryDetail[] = baseDeliveries.map((delivery, index) => {
...
export async function getDeliveries(filters: DeliveryListFilters): Promise<DeliveryListResult> {
  await delay(300)
...
export async function updateDeliveryStatus(
  ...
  const updated: DeliveryDetail = {
    ...current,
    status,
    deliveryDate: status === 'DELIVERED_CONFIRMED' ? new Date() : current.deliveryDate,
  }
